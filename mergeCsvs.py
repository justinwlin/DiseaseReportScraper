...
events = pd.read_csv("./data/H5N8_events.csv")
outbreaks = pd.read_csv("./data/H5N8_outbreaks.csv")
...
merged_df.rename(columns=lambda x: "event-" +
                 x if x in events.columns else "outbreak-" + x, inplace=True)

# Write the resulting merged dataframe to a new CSV file
merged_df.to_csv("./data/H5N8_merged.csv", index=False)
# merged_df.to_csv("./data/merged.csv", index=False)

# Print a message indicating that the file was saved successfully
print("Merged data saved to 'H5N8_merged.csv'")
